 ...
 */
class Rss20Parser extends BaseParser
{
    /**
    ...
    public function findFeedDate(SimpleXMLElement $xml, Feed $feed)
    {
        ...
        $published = !empty($publish_date) ? $this->dateParser->getDateTime(XmlParser::getValue($publish_date)) : null;
        $updated = !empty($update_date) ? $this->dateParser->getDateTime(XmlParser::getValue($update_date)) : null;

        if ($published === null && $updated === null) {
            $feed->setDate($this->dateParser->getCurrentDateTime()); // We use the current date if there is no date for the feed
    ...
    public function findItemPublishedDate(SimpleXMLElement $entry, Item $item, Feed $feed)
    {
        ...
        $item->setPublishedDate(!empty($date) ? $this->dateParser->getDateTime(XmlParser::getValue($date)) : null);
    }
